        ...
        schemeRefs.forEach(ref => {
          step = step.then(() => {
            ...
            if ((typeof entTagData !== 'string') && (ref._id === entScheme._id)) {
              entTagData = resolvedTerms
                .filter(t => (t.scheme_id === ref._id) && (t.vocabulary_type === 'class'))
                .map(t => `${t.scheme_id}_${t.vocabulary_label}_${t.label}`)
                .sort()
                .join('\r\n')
            }

            const tagData = resolvedTerms
              .filter(t => t.scheme_id === ref._id)
              .map(t => `${t.scheme_id}_${t.vocabulary_label}_${t.label}`)
              .sort()
              .join('\r\n')
